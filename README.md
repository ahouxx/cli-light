...
Blue=total   Green=idle   Orange=running (blink)   Red=needs approval
...
How It Works
Process scanner: Polls Get-Process every 3s — counts running claude.exe, kimi-cli.exe, opencode.exe
Hook server: HTTP on localhost:9876 — CLI hooks POST {state, agent} on state changes
Per-agent tracking: notify.ps1 walks the process tree to find the CLI PID, used as agent ID so multiple instances don't interfere
Hook Lifecycle
...
CLI Light Windows v0.2 · 桌面 CLI 状态灯
...
2. 安装 Hook（一次性）
powershell -ExecutionPolicy Bypass -File install.ps1
自动检测已安装的 CLI 工具并注册 hook，使其能上报状态变化。安装后重启 CLI 终端生效。
...
操作
拖动窗口移动位置
右键菜单 → 置顶 / 显示边框 / 主题 / 配色 / 缩放 / 样式（横向/竖向） / 退出
已知限制
